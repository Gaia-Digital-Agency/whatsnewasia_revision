...
import { GetTagByIdResponse, Tag } from "../../../types/tags.type";
import {
  deleteTag,
  getAllTags,
  getTagByID,
...
interface TableTagsProps {
  onEditTags: (tag: GetTagByIdResponse) => void;
  onRefresh: () => void;
  refreshKey: number;
}
...
export default function TableTags({
  ...
  const filteredAndSortedData = useMemo(() => {
    return dataTags.filter((item) =>
      [item.name, item.slug].some((value) =>
        value?.toLowerCase().includes(searchTerm.toLowerCase())
      )
    );
  }, [dataTags, searchTerm]); //sortKey, sortOrder
