...
const BE_HOST_URL = import.meta.env.VITE_WHATSNEW_BACKEND_URL
...
export default function Subscribers() {
  ...
  const [dataSubscriber, setDataSubscriber] =
  ...
  const fetchSubscribers = async (page = 1, limit = 5) => {
    ...
    try {
      ...
      const vaDataSubscriber = vaDataDetail?.subscribers as NewsletterSubscriber[];
      const vaDataPagination = vaDataDetail?.pagination as NewsletterPagination;
  ...
  const handleDonwload = async () => {
    ...
    // console.log("GAKA ADIL => " , response)
  };
                        ...
                        <TableRow key={item.id}>
                          ...
                          <TableCell className="px-4 py-4 font-normal text-gray-800 border border-gray-100 dark:border-white/[0.05] text-theme-sm dark:text-white/90 whitespace-nowrap">
                            <Badge
                              size="sm"
                              color={
                                item.status_subscription?.status === 1
                                  ? "success"
                                  : "warning"
                              }
                            >
                              {item.status_subscription?.description ?? "Unknown"}
                            </Badge>
                          </TableCell>
                          {/* <TableCell className="px-4 py-4 font-normal text-gray-800 border border-gray-100 dark:border-white/[0.05] text-theme-sm dark:text-white/90 whitespace-nowrap">
