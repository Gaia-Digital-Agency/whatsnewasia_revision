...
export default function UserDropdown() {
  ...
  async function handleSignOut() {
    try {
      // tembak API logout
      const credentials: LogoutCredentials = {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {myProfilePicture ? (
          <span className="mr-3 overflow-hidden rounded-full h-11 w-11 bg-gray-200 align-middle">
            <img
          ...
          <div className="bg-blue-500 h-11 w-11 rounded-full flex items-center justify-center text-2xl text-white mr-3"> 
            <h1>{myName.charAt(0).toUpperCase()}</h1>
          </div>
        )}

        <span className="block mr-1 font-medium text-theme-sm">{myName}</span>
        <svg
