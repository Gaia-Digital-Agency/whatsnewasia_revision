import { useEffect, useState } from "react";
import { 
  // ArrowUpIcon, 
  FileIcon, 
  GroupIcon } from "../../icons";
// import Badge from "../ui/badge/Badge";
import { getTotalSubscriber } from '../../services/newsletter.service';
import { getTotalArticles } from "../../services/article.service";

export default function FirstSection() {

  const [totalActiveSubscriber, setTotalActiveSubscriber] = useState<number>(0) ;
  const [totalPublishedArticle, setTotalPublishedArticle] = useState<number>(0) ;
  // const [totalNonActiveSubscriber, setTotalNonActiveSubscriber] = useState<number>(0) ;

  useEffect(() => {
    const countSubscribers = async () => {
      const getData = await getTotalSubscriber() ;
      if(getData){
        const dataSubscriber = getData.data ;
        const subsActive = dataSubscriber.active ;
        // const subsNonactive = dataSubscriber.non_active ;
        setTotalActiveSubscriber(subsActive) ;
        // setTotalNonActiveSubscriber(subsNonactive) ;
      }
    }

    const countArticles = async () => {
      const getData = await getTotalArticles() ;
      if(getData){
        const dataArticle = getData.data ;
        const nPublished = dataArticle.published ;
        setTotalPublishedArticle(nPublished ?? 0)
      }
    }

    countSubscribers() ;
    countArticles()
  },[])

  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 md:gap-6">
      {/* <!-- Metric Item Start --> */}
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
        <div className="flex items-center justify-center w-12 h-12 bg-gray-100 rounded-xl dark:bg-gray-800">
          <GroupIcon className="text-gray-800 size-6 dark:text-white/90" />
        </div>

        <div className="flex items-end justify-between mt-5">
          <div>
            <span className="text-sm text-gray-500 dark:text-gray-400">
              Newsletter Subscribers
            </span>
            <h4 className="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
              {totalActiveSubscriber}
            </h4>
          </div>
          {/* <Badge color="success">
            <ArrowUpIcon />
            11.01%
          </Badge> */}
        </div>
      </div>
      {/* <!-- Metric Item End --> */}

      {/* <!-- Metric Item Start --> */}
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
        <div className="flex items-center justify-center w-12 h-12 bg-gray-100 rounded-xl dark:bg-gray-800">
          <FileIcon className="text-gray-800 size-6 dark:text-white/90" />
        </div>

        <div className="flex items-end justify-between mt-5">
          <div>
            <span className="text-sm text-gray-500 dark:text-gray-400">
              Articles Published
            </span>
            <h4 className="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
              {totalPublishedArticle}
            </h4>
          </div>
          {/* <Badge color="success">
            <ArrowUpIcon />
            11.01%
          </Badge> */}
        </div>
      </div>
      {/* <!-- Metric Item End --> */}
    </div>
  );
}
